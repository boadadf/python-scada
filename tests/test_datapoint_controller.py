...
from frontend.tags.controller import DatapointController
...
def model():
    ...
    m._tags["Test@TAG"] = TagUpdateMsg(tag_id="Test@TAG", value=123, quality="good", timestamp="2025-09-15T12:00:00Z")
    return m
...
def test_publish_tag_blocks_when_initializing(controller):
    ...
    controller.publish_tag(TagUpdateMsg(tag_id="Test@TAG", value=456, quality="good", timestamp="2025-09-15T12:01:00Z"))
...
def test_publish_tag_emits_when_no_initializing(controller):
    ...
    assert args[1]["tag_id"] == "Test@TAG"

def test_handle_subscribe_live_feed_emits_initial_state(controller):
    with patch("frontend.tags.controller.join_room") as mock_join_room:
        controller.socketio.emit.reset_mock()
        ...
        for call_args in controller.socketio.emit.call_args_list:
            if call_args[0][0] == "initial_state":
                ...
                assert any(tag["tag_id"] == "Test@TAG" and tag["value"] == 123 for tag in tag_list)
                found = True
...
def test_set_tag_calls_service_and_emits_ack(controller):
    ...
    test_data = {"tag_id": "Test@TAG", "value": 42, "quality": "good", "timestamp": "2025-09-15T12:00:00Z"}
    ...
    controller.socketio.emit.assert_any_call('set_tag_ack', {"status": "ok", "tag_id": "Test@TAG"})